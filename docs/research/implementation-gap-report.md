...
foundation with authored schema policy and validation, but lacks a real immutable object-store and
projection lifecycle, graph merge semantics, and the typed agent/MCP retrieval plane. The project
should therefore be described as a local graph-VCS foundation, not as an agent retrieval system.
...
| Schema policy and validation | Partial | Canonical TOML schemas define node-label and edge-type rules, required typed properties, natural-key uniqueness, cardinality, and acyclic/self-loop invariants. `spl schema migrate` stages a schema and graph mutations atomically; `spl validate` reports immutable-snapshot violations. | Staging, commit, durable-open, and current merge lifecycle paths validate schemas. Historical snapshots retain their schema roots and remain readable. No public `fsck`, schema-aware query filtering, or merge-preview conflict report exists. |
...
| Merge lifecycle | Partial | Preview bindings, leases, conflict transaction state, recovery, abort/finalize primitives, and schema validation of clean/resolved snapshots exist. | No three-way graph-content merge, conflict classification/listing, or public command/tool performs the preview or merge. A "clean" merge retains the target snapshot rather than combining source changes. |
...
| CLI contract | Partial | `spl init`, add, status, commit, branch, switch, schema migrate, validate, resolve, diff, history, and branches-containing emit JSON. | No `merge`, `log`, conflict, query, search, filter, traversal, path, context, fsck, GC, or remote commands. |

## Prioritized gaps

| Priority | Gap | Research baseline | Current shortfall and consequence | Recommended next increment |
| --- | --- | --- | --- | --- |
| P0 | Actual graph merge semantics | v2 §10.4 and Milestone 0 acceptance | Merge metadata/lifecycle exists, but no merge engine computes a combined snapshot, change set, or conflicts. `ApplyCleanBoundMerge` creates a merge commit using the target snapshot. | Build deterministic merge-base diffing and three-way node/edge/property merge; return typed structural/schema/semantic conflicts and bind apply/finalize to the exact preview. |
...
1. **Foundation completion:** build the real three-way graph merge engine and expose preview and
   conflict inspection. Reuse the existing schema validator for merge-result enforcement.
