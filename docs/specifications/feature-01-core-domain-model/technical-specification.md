...
| --- | --- | --- |
| Store owns domain rules | `apps/store` must own entities, value objects, lifecycle transitions, invariant checks, domain errors, and persistence-facing ports for the core model. Controllers, DTOs, and adapters must not carry business rules. | Constitution III, IV; store AGENTS |
