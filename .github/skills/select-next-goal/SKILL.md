...
**If the capability being verified requires a human-authenticated session token** (i.e. any endpoint or MCP scenario gated behind a store-issued session token, per Meridian IDEA-81/G04), the Docker step's acceptance criteria must explicitly require obtaining that token via `pnpm dev:session-token` (`tools/dev-session-token/get-dev-session-token.mjs`) — which drives the real `GET /auth/github/login` → `GET /auth/github/callback` flow against the running containerized store (using the `github-oauth-stub` service already wired into `compose.yaml`) and prints a valid, usable session token. Never write the Docker step's acceptance criteria as if a token can be obtained by any other means (a client-supplied `actorKind`/`discipline` field, a hand-crafted or manually-signed token, skipping auth in the container, etc.) — a valid token minted through the real login/callback flow, via this tool, is the only way every auth-gated scenario (success path and every negative case) can be exercised faithfully against the running system. State this requirement explicitly in the goal file so `implement-next-goal` cannot substitute an easier shortcut.
