...
transaction. There is no separate "add myself" call needed — `POST /workspaces` alone creates the
workspace *and* makes the current user a member. Don't follow it with a redundant
...
1. **Obtain a session token for the current user.**
   - Real flow: `GET /auth/github/login` (redirects to GitHub) → `GET /auth/github/callback?code=&state=` →
     returns `{ sessionToken }` (`apps/store/src/auth/auth.controller.ts`).
