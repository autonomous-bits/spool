...
func NewRemoteCommand(repoProvider func() (*repository.Repository, error)) *cobra.Command {
	command := &cobra.Command{
		Use:   "remote",
		Short: "Configure a legacy Rack remote (deprecated / unsupported for solution context)",
		Long: "Deprecated and unsupported for solution context. Bind each code repo with .spool/context.toml (or `spl context init --remote <url>`) " +
			"and sync context with stock git clone, pull requests, and history. " +
			"Leftover `.spl`/Rack is migration-only (`spl context export`), not a parallel SoT. " +
